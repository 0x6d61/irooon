...
> **重要**: 作業を開始する前に `memory/` ディレクトリ内のメモファイルを確認してください。
> 既知の問題点、開発Tips、進捗が記録されています。
...
#### 0.x.0（マイナーアップデート）- 言語仕様が変わる大きな変更
- 新しい構文の追加（例: 型システム、演算子オーバーロード）
- 破壊的変更（例: キーワードの削除・変更）
- 大きな機能追加（例: CLR相互運用、クラス継承）

#### 0.x.y（パッチアップデート）
- バグ修正
- パフォーマンス改善
- ドキュメント修正
- 小さな機能改善（既存機能の拡張、標準ライブラリの追加メソッド）

**例:**
```
v0.5.0 - CLR相互運用 + クラス継承（大きな機能追加）
v0.5.1 - バグ修正、パフォーマンス改善
v0.5.2 - 標準ライブラリ拡充（小さな機能追加）
v0.6.0 - 型システム実装（言語仕様の大きな変更）
v0.6.1 - バグ修正
...
v1.0.0 - 言語仕様が安定した時点
```
